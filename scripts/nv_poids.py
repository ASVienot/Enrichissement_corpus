...
"""
Ce programme permet d'entrainer un modèle Naive Bayes avec modification de poids sur notre corpus d'origine et complet (totaldonnees.py) mais peut être aussi utilisé avec les corpus modifiés (corpus_annote.csv et A_1000.csv).
"""

# COMMANDE LES AMIS : python3 nv_poids.py
# tout simple
...
def entrainement():
    ...
    weights = np.where(train_corpus["accord"] == "D", 30, 1)
    ...
    model.fit(train_corpus['phrase'], train_corpus['accord'], sample_weight=weights)
...
def obtention_phrases(test_evaluation, test_corpus):
    ...
    with open("data_resultats/data_prediction_nv_poids.csv", "w") as fichier:
