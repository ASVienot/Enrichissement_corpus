...
"""Ce programme permet d'enrichir les annotations de notre corpus (et donc de rajouter des balises D)"""
...
for index, row in totaldata.iterrows():
    ...
    lemmes_str = str(row['lemmes']).lower()
    phrase = Phrase_csv(Tour=row['Tour'],
                    Id_phrase=row['Id_phrase'],
                    phrase=row['phrase'],
                    lemmes=lemmes_str,
...
with open("corpus_annote.csv", "w") as fichier:
